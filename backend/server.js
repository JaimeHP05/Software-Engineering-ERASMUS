        ...
        try {
            ...
            if (data.courses_list && data.courses_list.length > 0) {
                data.courses_list.forEach((pair, i) => {
                    ...
                    const destCred = pair.destination_credits || 6.0;
                    ...
                    const origCred = pair.origin_credits || 6.0;
...
app.post('/api/recommendations', (req, res) => {
    const { originCourseId, destUniId, degreeId } = req.body;
    ...
    db.get("SELECT name FROM courses WHERE id = ?", [originCourseId], (err, originCourse) => {
        ...
        // Basic filtering by University first
        let sqlDest = "SELECT id, name, credits FROM courses WHERE university_id = ?";
        let params = [destUniId];
        
        // If degree provided, filter strictly
        if (degreeId) {
             sqlDest += " AND degree_id = ?";
             params.push(degreeId);
        }

        db.all(sqlDest, params, (err, destCourses) => {
            if (err) return res.status(500).json({ message: 'DB Error' });
...
app.get('/api/admin/stats', (req, res) => {
    const stats = { statusCounts: {}, topDestinations: [] };
    db.all("SELECT status, COUNT(*) as count FROM agreements GROUP BY status", [], (err, rows) => {
        if (err) return res.status(500).json({ message: 'Error' });
        rows.forEach(row => { stats.statusCounts[row.status] = row.count; });
